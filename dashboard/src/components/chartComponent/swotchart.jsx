...
const SWOTFilter = ({ onFilter }) => {
    ...
    return (
      <div>
        <button onClick={() => handleFilter('Likelihood')}>Likelihood</button>
        <button onClick={() => handleFilter('Region')}>Region</button>
        <button onClick={() => handleFilter('Topics')}>Topics</button>
        <button onClick={() => handleFilter('relevance')}>relevance</button>
      </div>
    );
};
const Swotchart = () => {
    ...
    const filterData = (category) => {
        const filtered = data.filter(item => item.topic.toLowerCase() === category.toLowerCase());
        setFilteredData(filtered);
    };

    const renderChart = () => {
        const svg = d3.select('.chart-container');
        //svg1.selectAll('*').remove(); // Clear previous chart

        const margin = { top: 20, right: 20, bottom: 30, left: 40 };
        const width = 600 - margin.left - margin.right;
        const height = 400 - margin.top - margin.bottom;

        const xScale = d3.scaleBand().range([1, width]).padding(0.1);
        ...
        yScale.domain([0, d3.max(filteredData, d => d.intensity)]);
    ...
    return (
        <div>
            <h2>Dashboard</h2>
            <SWOTFilter onFilter={filterData} />
            <svg className="chart-container" width={600} height={400}></svg>
        </div>
    )
}
