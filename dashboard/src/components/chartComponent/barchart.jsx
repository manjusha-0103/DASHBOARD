...
const BarChart = (props) => {
  ...
  //   ...data.map(item =>[item.published.split(" ")[2],item.relevance,item.likelihood])
  // ]

  

  const chartData = [
    ['pestle','likelihood','relevence',],
...[...new Set(data.map(item => item.pestle))]
    .map(uniqueSource => {
        // Find the corresponding items for each unique source
        const itemsForSource = props.props.filter(item => item.pestle === uniqueSource);
        ...
        const aggregatedLikelihood = itemsForSource.reduce((sum, item) => sum + item.likelihood, 0);
        const aggregatedRelevance = itemsForSource.reduce((sum, item) => sum + item.relevance, 0);
        return [uniqueSource, aggregatedLikelihood,aggregatedRelevance];
    })
];
  return (
    <>
        ...
        <div className="bar-m">
              <Chart
                ...
                data={chartData}
                options={{
                  colors:["#5032a8","#a08be0"],
      
                  chart: {
                    title: "Published information with",
                    subtitle: "with respective pestle",
                  },
                  legend: { position: "top" },
                  is3D: true,
                }}
